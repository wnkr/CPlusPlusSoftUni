...
string ExecuteOperation(const string& command, const vector<vector<string>>& operations, const vector<int>& arrayNumbers)
{
	...
	for (size_t i = 0; i < operations.size(); i++)
	{
		for (size_t j = 0; j < operations[i].size(); j++)
		{
			...
			{
				outputStream << pointerOperations[i](arrayNumbers, startIndex, endIndex);
			}
		}
	}
